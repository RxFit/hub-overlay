...
- **An `unhandledRejection` listener — CORRECTED 2026-08-27, verified against the pinned `next@14.2.35`.** This section previously said to install none, reasoning that "Node ≥15 already defaults to `throw`, so a log-only listener would silently *downgrade* a crash to a swallow." That reasoning does not survive contact with the framework. `next/dist/server/lib/start-server.js` registers, unconditionally and in production (not dev-gated, and reached by `next start` via `dist/cli/next-start.js`):
  ```js
  process.on("uncaughtException", exception)
  process.on("unhandledRejection", exception)
  // const exception = (err) => { console.error(err) }  // "we keep the process alive"
  ```
  So Node's default-throw is **already** overridden by Next: an unhandled rejection neither crashes the process nor is ever promoted to an uncaught exception — which means `uncaughtExceptionMonitor` above **never sees one**. Omitting our listener does not preserve a crash; it leaves the whole class visible only as Next's unstructured, **unscrubbed** `console.error`. Ours is therefore purely additive: it cannot downgrade a crash that the framework already prevented, and it adds one scrubbed, fingerprinted record. **Residual, stated rather than hidden:** Next's raw `console.error` still prints first, so a rejection whose message embeds a secret is logged unscrubbed by the framework; removing Next's own listener to prevent that would change crash semantics and is out of scope.
