...
Today an unexpected error in a hub API route reaches, at best, an unstructured Cloud Run stdout line that nothing aggregates, alerts on, or persists; at worst it reaches nothing at all. Client-side errors reach nothing, ever. The root cause is singular and mechanical: **there is no route wrapper.** `grep` for `withErrorHandling|apiHandler|wrapHandler|withRoute` across `hub/` returns zero hits. Every one of the 111 exported handlers is on its own — its own try/catch (or none), its own logger choice, its own status code, its own response body — which is why 21 handlers have no catch at all, 33 catch blocks log nothing, and 58 distinct error-response shapes exist.
...
Everything in this section is a count or a `file:line` from the audit. Counts marked **✱** were **re-derived directly against the tree on 2026-08-24** and are load-bearing. Unmarked derived counts came from the 2026-08-20 audit pass and must be regenerated by `scripts/gen-fault-pending.mjs` before Phase 2 is scoped — do not treat them as exact.
...
| Metric | Value |
|---|---|
| API route files / exported handlers ✱ | **75 / 111** (GET 54, POST 41, DELETE 8, PATCH 5, PUT 3) |
| Route files containing **no** `catch` at all ✱ | **4** — `admin/semantic-brain-health`, `auth/[...nextauth]`, `cron/dispatch-alert`, `google/chat/me` |
| `createLogger(` call sites vs raw `console.*` calls ✱ | **38** vs **146** (74 in `app/`, 72 in `lib/`) — ~4× more unstructured logging than structured |
...
| `fetch(` sites in `app/` (client components + hooks) ✱ | **100** (**82**) — with **no** shared wrapper |
...
**`lib/logger.ts` must be refactored first.** `createLogger()` (`lib/logger.ts:7-18`) calls `pino(...)` fresh on every invocation, and in dev allocates a new `pino-pretty` stream each time. Calling it per request across 111 routes is a per-request logger-plus-stream allocation. Refactor to a single module-scope root `pino()` instance plus `root.child({ module, correlationId })`, preserving `createLogger`'s signature. `withFault` must never construct a pino instance per request.
...
### Layer 3 — Route handlers (111 handlers / 75 files) and Server Actions
...
- **No `unhandledRejection` listener.** Node ≥15 already defaults to `throw`, so a log-only listener would silently *downgrade* a crash to a swallow — strictly worse than nothing.
...
This is the single most important test in the plan, because it is what licenses shipping one wrapper to 111 handlers at once.
...
## Appendix A — Repo facts this document depends on (re-verified 2026-08-24)
