 ...
 */
export const GET = withFault('tool-artifacts', async (req: NextRequest) => {
 ...
 */
export const POST = withFault('tool-artifacts', async (req: NextRequest) => {
  const session = await getServerSession(authOptions)
  if (!session?.user) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }
 ...
 */
export const PATCH = withFault('tool-artifacts', async (req: NextRequest) => {
  const session = await getServerSession(authOptions)
  if (!session?.user) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }
 ...
 */
export const DELETE = withFault('tool-artifacts', async (req: NextRequest) => {
  const session = await getServerSession(authOptions)
  if (!session?.user) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }
