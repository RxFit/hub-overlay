...
export const POST = withFault('worker/jobs/[id]/result', async (req: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    // worker posts. Best-effort and idempotent — the panel's own save is a
    // no-op once this has run, and a failure here only defers the save to it.
    if (recordedWork && outcome.toolRun && body.status === 'ok' && isDeepToolId(outcome.toolRun.tool)) {
      const landed = outcome.toolRun
      void ensureDeepRunArtifactForRun(landed.id, landed.userEmail, getTenantId()).catch((err: unknown) => {
