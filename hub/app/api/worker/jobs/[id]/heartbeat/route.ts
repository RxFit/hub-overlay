 ...
 *
 * inspect2xx OFF: `200 { ok: false, reason: 'lease_lost' }` is this route's
 * PROTOCOL (the lease-guard verdict the worker keys on), not a smuggled
 * failure — the one legitimate shape the contract check would misread.
 */
export const POST = withFault(
  ...
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    try {
      ...
      // A silently-broken heartbeat surfaces later as phantom "worker went
      // silent" cancellations — let the wrapper log + report it.
      throw err
    }
  },
  { inspect2xx: false },
