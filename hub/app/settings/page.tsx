...
function KPIEditorCard({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <section className="settings-section" aria-label="Business KPIs">
      <h2 className="settings-section-title">
        <span className="rx-comment-label">01 //</span> Business KPIs
      </h2>
      <p className="settings-section-desc">
        Revenue and custom business metrics. Stored in Postgres — no spreadsheet needed.
        Operational Paperclip KPIs are auto-derived separately.
      </p>

      {error && (
        <div className="settings-test-result settings-test-result--error" style={{ marginBottom: '12px' }}>⚠️ {error}</div>
      )}

      {/* Existing KPIs */}
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {[1, 2, 3].map(i => <div key={i} style={{ height: '44px', borderRadius: '6px', background: 'var(--surface-2)', animation: 'pulse 1.5s ease-in-out infinite' }} />)}
        </div>
      ) : kpis.length === 0 ? (
        <div className="settings-empty" style={{ marginBottom: '12px' }}>No KPIs yet. Add your first one below.</div>
      ) : (
              ...
              {editingId === kpi.id ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '6px' }}>
                    <input style={inputStyle} placeholder="Label" value={editDraft.label ?? kpi.label}
                      onChange={e => setEditDraft(p => ({ ...p, label: e.target.value }))} />
                    <input style={inputStyle} placeholder="Value" value={editDraft.value ?? kpi.value}
                      onChange={e => setEditDraft(p => ({ ...p, value: e.target.value }))} />
                    <input style={inputStyle} placeholder="Trend (e.g. +12%)" value={editDraft.trend ?? (kpi.trend || '')}
                      onChange={e => setEditDraft(p => ({ ...p, trend: e.target.value }))} />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px' }}>
                    <select style={selectStyle} value={editDraft.trendDirection ?? (kpi.trendDirection || 'neutral')}
                      onChange={e => setEditDraft(p => ({ ...p, trendDirection: e.target.value as KpiDraft['trendDirection'] }))}>
                      {DIRECTION_OPTS.map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
                    </select>
                    <input style={inputStyle} placeholder="Unit (%/$)" value={editDraft.unit ?? (kpi.unit || '')}
                      onChange={e => setEditDraft(p => ({ ...p, unit: e.target.value }))} />
                    <select style={selectStyle} value={editDraft.visibility ?? (kpi.visibility || 'staff')}
                      onChange={e => setEditDraft(p => ({ ...p, visibility: e.target.value as KpiDraft['visibility'] }))}>
                      {VIS_OPTS.map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
                    </select>
                  </div>
                  <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                    <button onClick={() => { setEditingId(null); setEditDraft({}) }}
                      style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: '5px', color: 'var(--text-muted)', cursor: 'pointer', padding: '4px 12px', fontSize: '0.72rem' }}>
                      Cancel
                    </button>
                    <button onClick={() => handleSaveEdit(kpi.id)} disabled={saving === kpi.id}
                      style={{ background: 'var(--accent)', border: 'none', borderRadius: '5px', color: '#000', fontWeight: 700, cursor: saving === kpi.id ? 'wait' : 'pointer', padding: '4px 14px', fontSize: '0.72rem', opacity: saving === kpi.id ? 0.6 : 1 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: '120px' }}>
                    ...
                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '2px', fontFamily: 'var(--font-mono)' }}>
                      ...
                      {kpi.trend ? ` · ${kpi.trend}` : ''}
                      {' · '}
                      <span style={{ color: kpi.trendDirection === 'up' ? 'var(--accent)' : kpi.trendDirection === 'down' ? 'var(--danger)' : 'var(--text-muted)' }}>
                        {kpi.trendDirection === 'up' ? '↑' : kpi.trendDirection === 'down' ? '↓' : '→'}
                      </span>
                      {' · '}{kpi.visibility ?? 'staff'}
                    </div>
                  </div>
                  {isAdmin && (
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                      <button onClick={() => { setEditingId(kpi.id); setEditDraft({}) }}
                        style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: '5px', color: 'var(--text-muted)', cursor: 'pointer', padding: '3px 10px', fontSize: '0.68rem' }}>
                        Edit
                      </button>
                      <button onClick={() => handleDelete(kpi.id)} disabled={deleting === kpi.id}
                        style={{ background: 'transparent', border: '1px solid var(--danger, #e74c3c)', borderRadius: '5px', color: 'var(--danger, #e74c3c)', cursor: deleting === kpi.id ? 'wait' : 'pointer', padding: '3px 10px', fontSize: '0.68rem', opacity: deleting === kpi.id ? 0.5 : 1 }}>
                        {deleting === kpi.id ? '…' : '✕'}
                      </button>
                    </div>
                  )}
      ...
      {isAdmin && (
        <div style={{ border: '1px dashed var(--border)', borderRadius: '8px', padding: '12px', marginTop: '4px' }}>
          <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>Add KPI</div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '6px', marginBottom: '6px' }}>
            <input id="kpi-new-label" style={inputStyle} placeholder="Label (e.g. Monthly Revenue)" value={newDraft.label}
              onChange={e => setNewDraft(p => ({ ...p, label: e.target.value }))}
              onKeyDown={e => { if (e.key === 'Enter') handleAdd() }} />
            <input id="kpi-new-value" style={inputStyle} placeholder="Value (e.g. $42k)" value={newDraft.value}
              onChange={e => setNewDraft(p => ({ ...p, value: e.target.value }))} />
            <input id="kpi-new-trend" style={inputStyle} placeholder="Trend (e.g. +12%)" value={newDraft.trend}
              onChange={e => setNewDraft(p => ({ ...p, trend: e.target.value }))} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: '6px', alignItems: 'center' }}>
            <select id="kpi-new-direction" style={selectStyle} value={newDraft.trendDirection}
              onChange={e => setNewDraft(p => ({ ...p, trendDirection: e.target.value as KpiDraft['trendDirection'] }))}>
              {DIRECTION_OPTS.map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
            </select>
            <input id="kpi-new-unit" style={inputStyle} placeholder="Unit (% / $)" value={newDraft.unit}
              onChange={e => setNewDraft(p => ({ ...p, unit: e.target.value }))} />
            <select id="kpi-new-visibility" style={selectStyle} value={newDraft.visibility}
              onChange={e => setNewDraft(p => ({ ...p, visibility: e.target.value as KpiDraft['visibility'] }))}>
              {VIS_OPTS.map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
            </select>
            <button id="kpi-add-btn" onClick={handleAdd} disabled={adding || !newDraft.label.trim()}
              style={{ background: 'var(--accent)', border: 'none', borderRadius: '6px', color: '#000', fontWeight: 700, cursor: adding || !newDraft.label.trim() ? 'not-allowed' : 'pointer', padding: '6px 14px', fontSize: '0.75rem', opacity: adding || !newDraft.label.trim() ? 0.5 : 1, whiteSpace: 'nowrap', transition: 'opacity 0.15s ease' }}>
              {adding ? '…' : '+ Add'}
            </button>
          </div>
