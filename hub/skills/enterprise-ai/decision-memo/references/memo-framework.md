...
description: Builds a 1-page decision memo (context → options → recommendation → risks → ask) enforcing McKinsey memo framework — brutal brevity, SCP storyline clarity, decision-forcing output, evidence density. Use when drafting a memo for a leadership decision, a scope cut, a buy/build/partner call, a roadmap slip, or any "should we X?" moment that needs a yes/no from a specific person by a specific date.
---
...
## Memo Framework — four non-negotiables

Every decision memo must pass all four. If the draft fails one, rewrite. Do not ship a memo that fails the Memo Framework.
...
2. **Draft in Memo Framework order.** Title → Context → Complication → Options → Recommendation → Risks → Ask.

3. **Run the four tests.** For each section, confirm it passes the Memo Framework. Flag any that fail.
