...
describe('cooldown re-entry (Claude chain)', () => {
  it('skips a cooled-down model, then retries it once the cooldown expires', async () => {
    ...
    const r1 = await collect(streamChat(MESSAGES, 'sys', 'exa_search', false))
    ...
    const r2 = await collect(streamChat(MESSAGES, 'sys', 'exa_search', false))
    ...
    const r3 = await collect(streamChat(MESSAGES, 'sys', 'exa_search', false))
