...
| **4 · Reborn tooling + agentic panel** | executing | Re-point Interview Mode, the score-context gate, Pre-Cog, and the skills loader from "assemble a REST payload" to "brief and verify an `agy` run". Panel arc: `hub/docs/architecture/PHASE4_AGENTIC_PANEL_2026-09-05.md` — PR 1 shipped (the assistant reads the Hub's own execution ledgers via `hub/lib/execution-context.ts`; card taps attach the record; Pulse tab on live data); PR 2 shipped (the needs-you queue at the top of the Runs tab — `hub/lib/needs-you.ts`, derived from the same ledgers, with Explain / Retry / Dismiss; deep-run Retry re-enters `hub/lib/deep-run-start.ts`, the one start path). Next: playbooks catalog, scorecards |
