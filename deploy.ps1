...
if ($currentProject -ne $GCP_PROJECT) {
  Write-Host " WARNING: current=$currentProject, target=$GCP_PROJECT" -ForegroundColor Yellow
  Write-Host "       (deploy command explicitly targets $GCP_PROJECT)" -ForegroundColor Gray
}
Write-Host " [OK] targeting $GCP_PROJECT" -ForegroundColor Green

# 3. Verify source directory exists
Write-Host "  [3/4] Source directory..." -NoNewline
if (-not (Test-Path $SOURCE_DIR)) {
  Write-Host " [FAILED] $SOURCE_DIR not found" -ForegroundColor Red
  exit 1
}
Write-Host " [OK] $SOURCE_DIR" -ForegroundColor Green
...
Write-Host " [OK] $($ENV_VARS.Count) vars configured" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "`n[FAILED] Deploy failed!" -ForegroundColor Red
  exit 1
}

Write-Host "`n[SUCCESS] Deploy successful!" -ForegroundColor Green
...
if ($logs -match "OAuth config") {
  ...
  Write-Host "  WARNING: OAuth config log not found yet (may take a moment)" -ForegroundColor Yellow
...
if ($SkipQA) {
  Write-Host "`n* Skipping QA (use without -SkipQA to run post-deploy tests)" -ForegroundColor Yellow
} else {
  ...
  if (Test-Path $qaScript) {
    ...
    Write-Host "  WARNING: QA script not found at $qaScript" -ForegroundColor Yellow
    Write-Host "  Skipping automated QA." -ForegroundColor Gray
  }
}

Write-Host "`n==============================================" -ForegroundColor DarkGray
Write-Host "  [SUCCESS] Deploy pipeline complete" -ForegroundColor Green
